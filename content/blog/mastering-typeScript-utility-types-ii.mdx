---
title: 'Mastering TypeScript Utility Types: Part 2 — The Transformers & Type Thieves'
description: 'From brittle test mocks to type-safe state machines—how Partial, Exclude, ReturnType, and Parameters eliminate manual type definitions.'
date: 2026-01-24
tags: ['typescript', 'web development', 'react', 'nextjs']
---

If you recall in [Part 1](/blog/mastering-ts-utility-1), we covered the basics—`Record`, `NonNullable`, `Pick`, and `Omit`. The "sculptors" that help us shape our types.
...
Part 2 is all about the **Transformers & Type Thieves** so to say🤠. Utilities that automatically change existing types or straight-up "steal" type information from our functions.
...
export const runTest = async (
  testName: string,
  testFn: () => Promise<void>,
  options?: Partial<TestOptions>, // ← Magic happens here
): Promise<TestResult> => {
  const finalOptions = {
    ...defaultTestOptions,
    ...options,
...
**The Cheat Sheet Memory Aid:**

- `Exclude<T, U>` → The Filter. "Give me everything except these."
- `Extract<T, U>` → The Magnet. "Give me only what matches this pattern."
...
### 5. [`ReturnType<T>`](https://www.typescriptlang.org/docs/handbook/utility-types.html#returntypetype) - "The Return Stealer" 🕵️
...
// ❌ Duplication. Change the function? Update this interface too.
```
...
### 6. [`Parameters<T>`](https://www.typescriptlang.org/docs/handbook/utility-types.html#parameterstype) - "The Arguments Thief" 🎯
...
- ✅ `Partial<T>` for flexible APIs and tests
- ✅ `Required<T>` for validation
- ✅ `Exclude<T, U>` for filtering unions
- ✅ `Extract<T, U>` for selecting from unions
- ✅ `ReturnType<T>` for stealing return types
- ✅ `Parameters<T>` for stealing param types
...
1. 🔍 Find one test that needs all params → Use `Partial<T>`
2. 🎯 Replace one manual interface → Use `ReturnType<typeof fn>`
3. 🚫 Find one union that needs filtering → Use `Exclude<T, U>`
...
_Questions? Using these in production? Let me know in the comments!_ 🚀
