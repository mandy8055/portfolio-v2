---
title: 'Mastering TypeScript Utility Types: Part 1 — The Sculptors'
description: 'From "vibe-based" objects to optional chaining hell—how to use Record, NonNullable, Pick, and Omit to clean up your production codebase.'
date: 2026-01-05
...
1. Is `string` `null` or `undefined`? No. → Keep it.
2. Is `null` `null` or `undefined`? Yes. → Discard it.
3. Is `undefined` `null` or `undefined`? Yes. → Discard it.
...
Just like we use salt and sugar together to prepare delicious [Gujarati Dal](https://www.vegrecipesofindia.com/gujarati-dal-recipe/) 😋
