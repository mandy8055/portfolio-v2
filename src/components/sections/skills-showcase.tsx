...
export function SkillsShowcase() {
  ...
  // Split skills into rows for better visual layout
  const getSkillRows = () => {
    const totalSkills = allSkills.length;
    const row1Count = Math.ceil(totalSkills * 0.25);
    const row2Count = Math.ceil(totalSkills * 0.28);
    const row3Count = Math.ceil(totalSkills * 0.27);

    return [
      allSkills.slice(0, row1Count),
      allSkills.slice(row1Count, row1Count + row2Count),
      allSkills.slice(row1Count + row2Count, row1Count + row2Count + row3Count),
      allSkills.slice(row1Count + row2Count + row3Count),
    ];
  };
